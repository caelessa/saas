...
def cobrancas():
 ...
 # Hotfix de baixa manual: durante testes (e também em operações com PIX/dinheiro
 # conferido fora do Frota Fácil), a locadora pode registrar o pagamento sem
 # enviar WhatsApp nem exigir comprovante. A opção é exibida apenas para
 # contratos realmente Ativos e de periodicidade semanal.
 hoje_local=datetime.now(_tenant_zone(tid())).date()
 inicio_semana=hoje_local-timedelta(days=hoje_local.weekday())
 ...
 for item in items:
  ...
  if c.status!='Ativo':
   continue
  audits_semana=BillingAudit.query.filter(
  ...
  audit_semana=next((a for a in audits_semana if (a.payment_status or '').upper()=='PAGO'),None)
  if not audit_semana and audits_semana:
   audit_semana=audits_semana[0]
  baixas_manuais.append({'contract':c,'info':item['info'],'audit':audit_semana})

 pagina=render_template('cobrancas.html',items=items,hoje=hoje,auditoria=auditoria,tenant=tenant)
 if baixas_manuais:
  linhas=[]
  for x in baixas_manuais:
   c=x['contract']; info=x['info']; audit=x['audit']
   pago=bool(audit and (audit.payment_status or '').upper()=='PAGO')
   veiculo=html.escape(((c.vehicle.marca_modelo if c.vehicle else '') or 'Veículo'))
   placa=html.escape(((c.vehicle.placa if c.vehicle else '') or ''))
   motorista=html.escape(((c.driver.nome if c.driver else '') or ''))
   numero=html.escape(str(c.numero or c.id))
   total=moeda_br(info.get('total') or 0)
   if pago:
    acao='<span class=\"badge bg-success\">Pago nesta semana</span>'
   else:
    url_baixa=url_for('baixar_cobranca_manual',id=c.id)
    acao=(f'<form method=\"post\" action=\"{url_baixa}\" style=\"margin:0\" '
          f'onsubmit=\"return confirm(\'Confirmar baixa manual de R$ {total} para {veiculo} {placa}?\')\">'
          '<button class=\"btn btn-success btn-sm\" type=\"submit\">✓ Dar baixa manual</button></form>')
   linhas.append(
    '<tr>'
    f'<td><strong>{numero}</strong></td>'
    f'<td>{veiculo}<br><small class=\"text-muted\">{placa}</small></td>'
    f'<td>{motorista}</td>'
  ...
  painel=(
   '<section class=\"card shadow-sm mt-4 mb-4\" id=\"baixa-manual-cobrancas\">'
   '<div class=\"card-body\">'
   '<div class=\"d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2\">'
   '<div><h5 class=\"card-title mb-1\">✓ Baixa manual · contratos ativos</h5>'
   '<div class=\"text-muted small\">Registra o pagamento sem enviar mensagem e sem exigir comprovante. '
   'Os painéis financeiros são atualizados normalmente.</div></div>'
   f'<span class="badge bg-light text-dark">Semana {inicio_semana.strftime('%d/%m')}–{fim_semana.strftime('%d/%m/%Y')}</span>'
   '</div>'
   '<div class=\"table-responsive\"><table class=\"table table-sm align-middle mb-0\">'
   '<thead><tr><th>Contrato</th><th>Veículo</th><th>Motorista</th><th>Total</th><th>Ação</th></tr></thead>'
   '<tbody>'+''.join(linhas)+'</tbody></table></div>'
   '</div></section>'
  )
  if '</main>' in pagina:
   pagina=pagina.replace('</main>',painel+'</main>',1)
  elif '</body>' in pagina:
   pagina=pagina.replace('</body>',painel+'</body>',1)
  else:
   pagina+=painel
 return pagina
...
@app.route('/cobrancas/<int:id>/baixa-manual',methods=['POST'])
@login_required
def baixar_cobranca_manual(id):
 c=Contract.query.options(joinedload(Contract.driver),joinedload(Contract.vehicle)).filter_by(id=id,tenant_id=tid()).first_or_404()
 if c.status!='Ativo':
  flash('A baixa manual está disponível somente para contratos ativos.','warning')
  return redirect(url_for('cobrancas'))
 periodicidade=unicodedata.normalize('NFKD',str(c.periodicidade or '')).encode('ascii','ignore').decode('ascii').lower()
 if periodicidade and 'seman' not in periodicidade:
  flash('A baixa manual desta tela está disponível somente para cobranças semanais.','warning')
  return redirect(url_for('cobrancas'))

 hoje=datetime.now(_tenant_zone(c.tenant_id)).date()
 ...
 audit=next((a for a in audits_semana if (a.payment_status or '').upper()=='PAGO'),None)
 if audit:
  flash('Esta cobrança já está marcada como paga nesta semana.','info')
  return redirect(url_for('cobrancas')+'#baixa-manual-cobrancas')
 audit=audits_semana[0] if audits_semana else None
 info=calcular_cobranca_semanal(c)
 if audit is None:
  audit=BillingAudit(
   ...
   billing_date=hoje,base_amount=info['valor_base'],
   km_period=info.get('km_periodo'),km_limit=info.get('limite_km'),
   km_excess=info.get('km_excedente') or 0,
   excess_rate=c.valor_km_excedente or 0,
   excess_amount=info.get('valor_excesso') or 0,total_amount=info['total'],
   body='Baixa manual sem envio de cobrança.',template_name=None,provider='manual',
   status='BAIXA_MANUAL',payment_status='PENDENTE',created_at=agora_sao_paulo_naive(),
  )
  db.session.add(audit)
  db.session.flush()
 else:
  # Atualiza os valores calculados antes da baixa, sem criar outra auditoria.
  audit.base_amount=info['valor_base']
  audit.km_period=info.get('km_periodo')
  audit.km_limit=info.get('limite_km')
  audit.km_excess=info.get('km_excedente') or 0
  audit.excess_rate=c.valor_km_excedente or 0
  audit.excess_amount=info.get('valor_excesso') or 0
  audit.total_amount=info['total']
 ...
 audit.closed_at=agora
 audit.status='BAIXA_MANUAL'

 # Se existir algum lembrete ainda agendado para esta auditoria, cancela para
 # garantir que uma baixa manual nunca seja seguida por cobrança automática.
 filas=MessageQueue.query.filter(
  MessageQueue.tenant_id==c.tenant_id,
  MessageQueue.related_entity=='Cobranca',
  MessageQueue.related_entity_id==audit.id,
  MessageQueue.status.in_(['AGENDADA','AGUARDANDO_MANUAL']),
 ).all()
 for fila in filas:
  fila.status='CANCELADA'
  fila.updated_at=agora
  db.session.add(MessageEvent(tenant_id=fila.tenant_id,message_id=fila.id,event='CANCELADA',description='Lembrete cancelado após baixa manual da cobrança.',created_at=agora))

 db.session.commit()
 flash(f'Baixa manual registrada: R$ {moeda_br(audit.total_amount or 0)}. Nenhuma mensagem foi enviada.','success')
 return redirect(url_for('cobrancas')+'#baixa-manual-cobrancas')
