...
def tenant_backup_payload(tenant_id):
 ...
 models=[Driver,Investor,InvestorVehicleRule,Vehicle,Odometer,MileageRequest,ContractTemplate,Contract,ContractEvent,Document,Maintenance,Inspection,Alert,Integration,MessageQueue,MessageEvent,BillingAudit]
 return {
...
def resumo_dashboard_proprietario(investor_id, inicio, fim):
 """Resumo financeiro/operacional por período, isolado por tenant e proprietário."""
 investor=Investor.query.filter_by(id=investor_id,tenant_id=tid()).first_or_404()
 veiculos=Vehicle.query.filter_by(tenant_id=tid(),investor_id=investor.id).order_by(Vehicle.placa).all()
 ids=[v.id for v in veiculos]
 por_veiculo={v.id:{
  'vehicle':v,'receita':Decimal('0'),'repasse':Decimal('0'),'custos':Decimal('0'),
  'resultado':Decimal('0'),'aberto':Decimal('0')
 } for v in veiculos}
 mensal={}

 contratos=Contract.query.filter(Contract.tenant_id==tid(),Contract.vehicle_id.in_(ids or [-1])).all()
 contratos_por_id={c.id:c for c in contratos}
 audits=BillingAudit.query.filter(
  BillingAudit.tenant_id==tid(),
  BillingAudit.contract_id.in_(list(contratos_por_id.keys()) or [-1]),
  BillingAudit.billing_date>=inicio,
  BillingAudit.billing_date<=fim
 ).order_by(BillingAudit.billing_date).all()

 for a in audits:
  c=contratos_por_id.get(a.contract_id)
  if not c or c.vehicle_id not in por_veiculo:
   continue
  total=Decimal(str(a.total_amount or 0))
  row=por_veiculo[c.vehicle_id]
  chave=a.billing_date.strftime('%Y-%m')
  mensal.setdefault(chave,{'repasse':Decimal('0'),'custos':Decimal('0'),'resultado':Decimal('0')})
  if (a.payment_status or '').upper()=='PAGO':
   regra=regra_proprietario_veiculo(c.vehicle_id,a.billing_date)
   pct=Decimal(str(regra.percentual_proprietario or 0)) if regra else Decimal('100')
   repasse=total*pct/Decimal('100')
   row['receita']+=total
   row['repasse']+=repasse
   mensal[chave]['repasse']+=repasse
  else:
   row['aberto']+=total

 manutencoes=Maintenance.query.filter(
  Maintenance.tenant_id==tid(),
  Maintenance.vehicle_id.in_(ids or [-1])
 ).all()
 for m in manutencoes:
  try:
   data_m=datetime.strptime(m.data,'%Y-%m-%d').date()
  except Exception:
   continue
  if data_m<inicio or data_m>fim or m.vehicle_id not in por_veiculo:
   continue
  custo=Decimal(str(m.custo or 0))
  por_veiculo[m.vehicle_id]['custos']+=custo
  chave=data_m.strftime('%Y-%m')
  mensal.setdefault(chave,{'repasse':Decimal('0'),'custos':Decimal('0'),'resultado':Decimal('0')})
  mensal[chave]['custos']+=custo

 total_receita=total_repasse=total_custos=total_aberto=Decimal('0')
 linhas=[]
 for v in veiculos:
  row=por_veiculo[v.id]
  ...
  row['motorista']=motorista_atual_veiculo(v)
  row['contrato']=Contract.query.filter(
   Contract.tenant_id==tid(),Contract.vehicle_id==v.id,
   Contract.status.in_(['Gerado','Enviado','Visualizado','Assinado','Ativo'])
  ).order_by(Contract.id.desc()).first()
  row['manutencao']=Maintenance.query.filter(
   Maintenance.tenant_id==tid(),Maintenance.vehicle_id==v.id,Maintenance.status!='Concluída'
  ).order_by(Maintenance.id.desc()).first()
  row['alertas']=Alert.query.filter_by(tenant_id=tid(),entidade='vehicle',entidade_id=v.id).filter(Alert.resolvido_em.is_(None)).count()
  linhas.append(row)
 ...
 while cursor<=limite:
  ...
  item=mensal.get(chave,{'repasse':Decimal('0'),'custos':Decimal('0'),'resultado':Decimal('0')})
  item['resultado']=item['repasse']-item['custos']
  meses.append({'label':cursor.strftime('%m/%Y'),'repasse':float(item['repasse']),'custos':float(item['custos']),'resultado':float(item['resultado'])})
  cursor=date(cursor.year+1,1,1) if cursor.month==12 else date(cursor.year,cursor.month+1,1)

 return {
  'investor':investor,'veiculos':linhas,'meses':meses,
  'totais':{
   'receita':total_receita,'repasse':total_repasse,'custos':total_custos,
   'resultado':total_repasse-total_custos,'aberto':total_aberto
  }
 }
...
def dashboard_investidor(id):
 hoje=date.today()
 try: inicio=datetime.strptime(request.args.get('inicio') or f'{hoje.year}-01-01','%Y-%m-%d').date()
 except Exception: inicio=date(hoje.year,1,1)
 try: fim=datetime.strptime(request.args.get('fim') or hoje.isoformat(),'%Y-%m-%d').date()
 except Exception: fim=hoje
 if inicio>fim:
  inicio,fim=fim,inicio
 dados=resumo_dashboard_proprietario(id,inicio,fim)
 return render_template('proprietario_dashboard.html',inicio=inicio.isoformat(),fim=fim.isoformat(),**dados)
